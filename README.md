...
- Live site: https://running-client-list-cbd69015c7bc.herokuapp.com/ {:target="\_blank"}
- Googlesheets link: https://docs.google.com/spreadsheets/d/17M0THf5HOCmTCS_M8pkf5eGf52iwv_LcsSCPS_9-gNo/edit?usp=sharing {:target="\_blank"}
...
As demonstated in the workflow image above, this app has five main features:
...
4. Delete a client - the user can search for a client using their email address. The client's data will be displayed and the user will be asked if they are sure they want to delete the information and to choose Y or N.
...
- I would like to create input and automated calculations options for other sports such as cycling, swimming, weight-lifting etc depending on the trainer's area of expertise and their clients' needs. This would involve optimizing the code into class objects for the new client and the specific sport or sports that they practice.
...
- Code Institute's PEP8 Checker - to test and validate the python code
...
3. The user can choose to type a number or type out their choice e.g. "1" or "Add a client". It is insensitive, so the user can type in all lower case, or in capitals, or in a mix of the two, with no errors arising.
...
2. If the user types only one name, they will see a warning saying their input does not match the specified pattern. They will be prompted to input a valid name.
...
The user must include an "@" symbol and a "." for the address to be valid, otherwise they will be warned and prompted to input a valid email address.
...
2. If the user inputs an invalid email, for example, with the @ or "." missing, they will be warned of the error and prompted to input a valid email.
...
4. When the user inputs a valid email that exists in the database, the "Days until the next race" figure will be updated from today. The client's information will then be displayed and the Main Menu will also be displayed.
...
- PyInputPlus information and explanations - https://pyinputplus.readthedocs.io/en/latest/ , https://dev.to/dominickoech/input-validation-using-pyinputplus-module-in-python-455p, https://stackoverflow.com/questions/66494597/pyinputplus-allowregexes-keyword-allows-any-input
- For an explanation of the index() method - https://www.programiz.com/python-programming/methods/list/index
...
- I'd like to thank my mentor Rohit Sharma for his time and invaluable advise on industry standards and best practices. He is always supportive and gives fantastic tips on how to make my projects as optimal as possible within the project time constraints and my own coding ability constraints!
- And thank you to the CI tutor Sarah who was very helpful when I had issues deploying the project to Heroku for the first time.
